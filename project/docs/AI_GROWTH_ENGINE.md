...
Текущая стадия — OPPORTUNITY_ADVISORY. Реализован путь RawSearchQuery → versioned V2 intelligence → repository site coverage → агрегированные Opportunity. Результаты относятся только к текущей выборке seed «создание сайтов». Автоматическое изменение production отсутствует; все действия требуют approval.
